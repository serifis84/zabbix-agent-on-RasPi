...
# modified 2022 Grigoris Vassilopoulos
case "$1" in
        boardrevision)
        ...
        coreclock)
                ...
                /bin/vcgencmd measure_clock core | sed s/"frequency(1)="//g
                ;;
        cpuclock)
                # ARM Clock Speed in Hz
                /bin/vcgencmd measure_clock arm | cut -d "=" -f 2
                ;;
        h264clock)
                # H264 Clock Speed in Hz
                /bin/vcgencmd measure_clock h264 | sed s/"frequency(28)="//g
                ;;
        ispclock)
                # ISP Clock Speed in Hz
                /bin/vcgencmd measure_clock isp | cut -d "=" -f 2
                ;;
        v3dclock)
                # v3d Clock Speed in Hz
                /bin/vcgencmd measure_clock v3d | cut -d "=" -f 2
                ;;
        uartclock)
                # uart Clock Speed in Hz
                /bin/vcgencmd measure_clock uart | sed s/"frequency(22)="//g
                ;;
        pwmclock)
                # pwm Clock Speed in Hz
                /bin/vcgencmd measure_clock pwm | sed s/"frequency(25)="//g
                ;;
        emmcclock)
                # emmc Clock Speed in Hz
                /bin/vcgencmd measure_clock emmc | cut -d "=" -f 2
                ;;
        pixelclock)
                # pixel Clock Speed in Hz
                /bin/vcgencmd measure_clock pixel | sed s/"frequency(29)="//g
                ;;
        vecclock)
                # vec Clock Speed in Hz
                /bin/vcgencmd measure_clock vec | sed s/"frequency(10)="//g
                ;;
        hdmiclock)
                # hdmi Clock Speed in Hz
                /bin/vcgencmd measure_clock hdmi | cut -d "=" -f 2
                ;;
        dpiclock)
                # dpi Clock Speed in Hz
                /bin/vcgencmd measure_clock dpi | sed s/"frequency(4)="//g
                ;;
        cpumem)
                # CPU Memory in MByte
                /bin/vcgencmd get_mem arm | tr -d "arm=M"
                ;;
        cpuvoltage)
                # CPU Voltage in Volt
                /bin/vcgencmd measure_volts | tr -d "volt=V"
                ;;
        firmwareversion)
                # Just the naked String of the firmware Version
                /bin/vcgencmd version | grep version | cut -d " " -f 2
                ;;
        gpumem)
                # Graphics memeory in MByte
                /bin/vcgencmd get_mem gpu | tr -d "gpu=M"
                ;;
        sdcardfree)
        ...
        sdramcvoltage)
                # sdram_c Voltage in Volt
                /bin/vcgencmd measure_volts sdram_c| tr -d "volt=V"
                ;;
        sdramivoltage)
                # sdram_i Voltage in Volt
                /bin/vcgencmd measure_volts sdram_i| tr -d "volt=V"
                ;;
        sdrampvoltage)
                # sdram_p Voltage in Volt
                /bin/vcgencmd measure_volts sdram_p| tr -d "volt=V"
                ;;
        temperature)
                # Temperature in degree celsius
                /bin/vcgencmd measure_temp | tr -d "temp='C"
                ;;
        throttlestate)
                # throttle state, for more information: 
		        # https://www.raspberrypi.org/forums/viewtopic.php?f=63&t=147781&start=50#p972790
                /bin/vcgencmd get_throttled | sed s/"throttled="//g | perl -e '$number = hex(<STDIN>); printf "%.32b\n", $number'
                ;;
        *)
